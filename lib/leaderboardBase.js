...
class LeaderboardBase {
    ...
    static create(redis, lbname) {
        ...
        board._redis = redis;
        board._lbname = lbname;             // database for scores
        board._jsonStoreName = "JsonData";  // database for full JSON results
     ...
     * Get JsonStore name.
     * @public
     * @return {string} JsonStore name.
     * @description getter for _jsonStoreName.
     */
    get jsonStoreName() {
        return this._jsonStoreName;
    }
     ...
     */
    _setJSON(name, json) {
        //return this.redis.zaddAsync(this.jsonStoreName, json, name);
        return this.redis.rpushAsync(name, json);
    }
    ...
    setScore(name, score) {
        ...
        .then((data) => {
            ...
            if (data[0] > score) {
                //console.log("no need to replace the value");
                return;
            }
            //console.log("replacing the old value");
            return this._setScore(name, score);
        });
        //return this._setScore(name, score);
    }
     ...
     * @public
     * @param {string} name - player name
     * @return {string} hashedName //{Promise}
     */
    setName(name) {
        this.redis.get(name, function(err, reply) {
            if (reply == null) {
                var hw = hashwords();
                var str = hw.randomStr();
                this.redis.set(name, str);
                console.log(str);
                return str;
            } else {
     ...
     */
    setJSON(name, json) {
        return this._setJSON(name, json);
    }
